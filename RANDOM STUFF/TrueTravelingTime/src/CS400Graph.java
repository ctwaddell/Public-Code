...
// TA: Keren
// Lecturer: Gary
// Notes to Grader: I added two ways to check if the end vertex is disconnected and still failed the autograder
...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        /**
         * this toString() method is needed or else the tester loses its mind
         */
        @Override
        public String toString()
        {
          ...
          {
            r += dataSequence.get(i);
            if(i == dataSequence.size() - 1)
            {
              continue;
            }
            r += ", ";
    ...
    /**
     * pathScanner is a little helper method that tells the program which edges a given vertex has
     * @param v, vertex to scan edges for
     * @return LinkedList<Edge> representing the number of edges from a given vertex
     */
    private LinkedList<Edge> pathScanner(Vertex v)
    {
      LinkedList<Edge> returnList = new LinkedList<Edge>();
      for(int i = 0; i < v.edgesLeaving.size(); i++)
      {
        returnList.add(v.edgesLeaving.get(i));
      }
      return returnList;
    }
    ...
    {    
        ...
        {
          throw new NoSuchElementException("The start or end cannot be null");
        }
        Vertex current = vertices.get(start); //assign current vertex to starting vertex
        known.add(vertices.get(start)); //add start vertex to known      
        Path currentPath = new Path(current);
        //loop begins?
        while(true)
        {
          if(autoFail > vertices.size()) //if autofail ever becomes greater than the number of verticies, by properties of Djikstra's algorithim that means the node isn't connected.
          {
            throw new NoSuchElementException("There is no path to given endpoint");
          }
          if(currentPath.end.equals(vertices.get(end))) //if the end of currentPath is the search end, return the path
          {
            return currentPath;
          }
          
          boolean extraMileage = false;
          LinkedList<Edge> tempAdder = pathScanner(current); //find all paths for current vertex
          for(int i = 0; i < tempAdder.size(); i++)
          {
            if(known.contains(tempAdder.get(i).target)) //skip over known vertices
            {
              continue;
            }
            Path toAdd = new Path(currentPath, tempAdder.get(i)); //add edge to current path
            frontier.add(toAdd); //does add
            extraMileage = true;
          } //finished updating frontiers
          try
          {
            while(known.contains(frontier.peek().end)) //avoid making more than one path to same node
            {
              frontier.poll();
            }
          }
