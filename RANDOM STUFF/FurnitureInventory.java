 ...
 */
class Furniture implements Comparable {
  ...
  public Furniture(String name, int weight) {
    ...
    inventoryNumGenerator++;
    // CHANGE THIS, included only to avoid compile errors
  }
  ...
  public boolean equals(Object o) {
    ...
    {
      if(this.getName().equals(((Furniture) o).getName()))
      {
        return true;
      }
    }
...
public class FurnitureInventory {
  ...
  public FurnitureInventory() {
    this.list = new ArrayList<Furniture>(0);
  ...
  public static FurnitureInventory copyOf(FurnitureInventory toCopy) {
    ...
    // the same elements
    FurnitureInventory f = new FurnitureInventory();
    for(int i = 0; i < toCopy.list.size(); i++) 
    {
      f.list.add(toCopy.list.get(i));
    }
    return f; // CHANGE THIS, included only to avoid compile errors
  }
  ...
  public int getNumOccurrences(Furniture f) {
    ...
    return counter; // CHANGE THIS, included only to avoid compile errors
  }
  ...
  public int getHeaviestFurniture() {
    ...
    // below.
    this.sortInventory();
    int finalElement = this.list.size() - 1;
    Furniture f = this.list.get(finalElement);
    int id = f.getInventoryId();
    return id;
