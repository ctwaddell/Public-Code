...
*   1. isEmpty() 111
*   2. size() 111
*   3. addWinnerHelper() 111
*   4. search() 111 
*   5. updateRecordTime() 111
*   6. getMin() 111
*   7. deleteHelper()
*/
public class AutoRaceRecords {
  ...
  protected BSTNode<Winner> addWinnerHelper(Winner newWinner, BSTNode<Winner> node) {
    if(node == null)
    {
      BSTNode<Winner> thing = new BSTNode<Winner>(newWinner);
      size++;
      return thing;
    }
    ...
    if(comparison == 0)
    {
      throw new IllegalStateException("Duplicate element found in tree");
    }
    ...
    if(comparison < 0)
    {
      addWinnerHelper(newWinner, node.getLeft());
    }
    // else recursive case: recurse right 
    // try to insert newWinner to the right subtree
    if(comparison > 0)
    {
      addWinnerHelper(newWinner, node.getRight());
    }
  ...
  */
  ...
  public void updateRecordTime(int year, int time) {
    Winner toUpdate = null;
    try 
    {
      toUpdate = search(year);
    }
